...
xmlhttp6.open("GET","main.json")
xmlhttp6.send();
xmlhttp6.addEventListener("readystatechange",function(){
    if(this.readyState==4 && this.status==200){
        data=JSON.parse(xmlhttp6.response);
        Phone(0,9)
        cartPush ();
        }
     })
     ...
     function Phone(x,y ){
        ...
        document.getElementById("Phone").innerHTML=containerPhone;
...
if (data[index].title.toLowerCase().includes(term.toLowerCase().trim())==true ){
 ...
 document.getElementById("Phone").innerHTML=containerPhone2;
}
}
}
